...
try {
  ...
  await database.query("UPDATE accounts SET enabled = false WHERE id = $1", [accountRow.rows[0].id]);
  assert.equal((await request("/api/v1/auth/me", cookieA)).status, 401);
} finally {
  await database.query("UPDATE accounts SET enabled = true WHERE lower(username) = lower($1)", [accountA.username]);
  await database.end();
}
